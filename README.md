...
네이버 블로그 글 링크와 닉네임을 기준으로 댓글을 찾고, 선택한 댓글 또는 댓글/직접 입력 이메일에 해당하는 네이버 메일 본문을 PNG로 저장하는 Windows용 GUI 프로그램입니다.
...
4. GUI가 열리면 `네이버 로그인 열기`로 로그인한 뒤, 열린 Chrome 창을 그대로 둔 채 `찾고 캡처` 또는 `이메일 본문 캡처`를 사용합니다.
...
4. GUI에서 `네이버 로그인 열기`로 로그인한 뒤, 열린 Chrome 창을 그대로 둔 채 캡처 기능을 사용합니다.
